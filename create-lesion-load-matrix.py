...
def get_binary_lesion_imgs(data_dir):
    ...
    if not os.path.isfile(data_dir + "binary_imgs.npy"):
        ...
        dataset_path = data_dir + "stroke-dataset/HallymBundang_lesionmaps_Bzdok_n1401_old/"
...
def create_lesion_load_matrix(data_dir, lesion_data, reference_img, save_interm_labels=False):
    ...
    atlas_cort_vectorized, atlas_cort_labels = load_ho_cort_atlas(reference_img, mask)
    atlas_subcort_vectorized, atlas_subcort_labels = load_ho_subcort_atlas(reference_img, mask)
    atlas_cereb_vectorized, atlas_cereb_labels = load_cereb_atlas(atlas_dir, reference_img, mask)
    atlas_wm_vectorized, atlas_wm_labels = load_jhu_wm_atlas(atlas_dir, reference_img, mask)

    if save_interm_labels:

        np.save(atlas_dir + "processed/" + "ho_cortical_atlas_vectorized.npy", atlas_cort_vectorized)
        np.save(atlas_dir + "processed/" + "ho_cortical_atlas_region_labels.npy", atlas_cort_labels)

        np.save(atlas_dir + "processed/" + "ho_subcortical_atlas_vectorized.npy", atlas_subcort_vectorized)
        np.save(atlas_dir + "processed/" + "ho_subcortical_atlas_region_labels.npy", atlas_subcort_labels)

        np.save(atlas_dir + "processed/" + "cereb_atlas_vectorized.npy", atlas_cereb_vectorized)
    ...
    llm_cort, region_names_cort = create_lesion_load_matrix_atlas(atlas_cort_vectorized, atlas_cort_labels, lesion_data)
    llm_subcort, region_names_subcort = create_lesion_load_matrix_atlas(atlas_subcort_vectorized, atlas_subcort_labels, lesion_data)
    llm_cereb, region_names_cereb = create_lesion_load_matrix_atlas(atlas_cereb_vectorized, atlas_cereb_labels, lesion_data)
...
def main():
    ...
    lesion_load_matrix, region_labels = create_lesion_load_matrix(DATA_DIR, lesion_imgs, reference_img, False)

    print(lesion_load_matrix.shape, region_labels.shape)
    np.save(DATA_DIR + "llm/combined_lesions_load_matrix.npy", lesion_load_matrix)
    np.save(DATA_DIR + "llm/combined_atlas_region_labels.npy", region_labels)
